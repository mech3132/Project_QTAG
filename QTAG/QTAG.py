...
parser = argparse.ArgumentParser(
    description="Bins and classifies OTUs according to distribution of population across gradient.\n This script takes a taxa x sample table (tab-delimited) and determines whether each ASV/OTU is most abundanct in low, intermediate, or high gradient values. \n\n\n")
...
optional.add_argument(
    '--minSamplePres',
    help = 'Threshold for number of samples in which a taxa must appear. [default 3]',
    required = False,
    default = 3)
optional.add_argument(
    '--pvalthresh',
    ...
    def QTAG_get_tolerance_range(self, thresh, threshAbund):
        ...
        for taxa in list(self.allListAbund.keys()):
            ...
            for i in sorted(set(tempListAbund[1])):
                currRA=sum([ tempListAbund[0][x[0]] for x in enumerate(tempListAbund[1]) if x[1] <= i]) # Make approximate due to math errors?
...
#                         uprOccur=i
    ...
    def QTAG_plot_fit(self, overwrite, version):
        if (os.path.isdir(self.output_dir + '/plots_quantile')) and (not overwrite) and (version=='both' or version=='quantile'):
        ...
        if (os.path.isdir(self.output_dir + '/plots_simple')) and (not overwrite) and (version=='both' or version=='simple'):
            print("Plot directory already exists. To overwrite, include overwrite=True.")
        else:
...
#                    plt.plot(qcurves[0], qcurves[3])
    ...
    def QTAG_plot_class_summary(self, overwrite, version):
        if (os.path.isfile(self.output_dir + '/class_summary_plot_quantile.png')) and (not overwrite) * ( version == 'both' or version == 'quantile'):
        ...
        if (os.path.isfile(self.output_dir + '/class_summary_plot_simple.png')) and (not overwrite) * ( version == 'both' or version == 'simple'):
            print("Class summary plot already exists.To overwrite, include overwrite=True.")
        else:
...
# LOAD #
if overwrite or not os.path.isfile(output_dir):
    ...
    print("Error: output directory already exists. Will not overwrite existing data unless -overwrite is changed to False")
